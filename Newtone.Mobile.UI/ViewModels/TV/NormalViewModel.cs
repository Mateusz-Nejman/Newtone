...
using Newtone.Mobile.UI.Views.TV.Custom;
using Xamarin.Forms;
...
namespace Newtone.Mobile.UI.ViewModels.TV
{
    public class NormalViewModel : PropertyChangedBase
    {
        ...
        public ICommand GotoSettings
        {
            get
            {
                if (gotoSettingsCommand == null)
                    gotoSettingsCommand = new ActionCommand(async(parameter) =>
                    {
                        await Global.NavigationInstance.PushModalAsync(new ModalPage(new SettingsPage(), Localization.Settings));
                    });
        ...
        public ICommand GotoDownload
        {
            get
            {
                if (gotoDownloadCommand == null)
                    ...
                    {
                        await Global.NavigationInstance.PushModalAsync(new ModalPage(new Views.DownloadPage(), Localization.TitleDownloads));
                    });
        ...
        public ICommand GotoSearch
        {
            get
            {
                if (gotoSearchCommand == null)
                    gotoSearchCommand = new ActionCommand(async(parameter) =>
                    {
                        await Global.NavigationInstance.PushModalAsync(new ModalPage(new SearchPage(), Localization.Search,false));
                    });
        ...
        #endregion
        #region Constructors
        public NormalViewModel(Grid container, PlayerPanel panel, INFocusElement focusElementFromUp, INFocusElement focusElementFromDown)
        {
            FocusElementFromUp = focusElementFromUp;
            FocusElementFromDown = focusElementFromDown;
        ...
        private void SetContainer(ContentView content, string title)
        {
            ...
            if(content is INFocusContent focusContent)
            {
                FocusElementDown = focusContent.BottomElement;
                FocusElementUp = focusContent.TopElement;
                focusContent.TopElement.NextFocusUp = FocusElementFromUp;
                focusContent.BottomElement.NextFocusDown = FocusElementFromDown;
            }
