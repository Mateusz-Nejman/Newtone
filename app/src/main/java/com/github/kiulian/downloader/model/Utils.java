...
public class Utils {
    ...
    public static List<Thumbnail> parseThumbnails(JSONObject container) {
        if (container == null) {
            return null;
        }
        ...
        if (jsonThumbnails == null) {
            ...
            for (int i = 0; i < jsonThumbnails.size(); i++) {
                ...
                if (jsonThumbnail.containsKey("url")) {
                    thumbnails.add(new Thumbnail(jsonThumbnail.getInteger("width"), jsonThumbnail.getInteger("height"), jsonThumbnail.getString("url")));
                }
            }
            return thumbnails.stream().sorted((c1, c2) -> {
                int w1 = c1.width;
                int w2 = c2.height;
